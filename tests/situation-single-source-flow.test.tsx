...
describe('situation single-source flow', () => {
  ...
  it('shows filing mode without recommendation prefix when there is only one tax scenario', () => {
    ...
    const openAllScenariosButton = Array.from(container.querySelectorAll<HTMLButtonElement>('button'))
      .find((button) => button.textContent?.includes('查看所有稅額組合'))
    expect(openAllScenariosButton?.className).toContain('w-full')
  ...
  it('opens tax bracket reference from summary scenario dialog', () => {
    ...
    clickButtonByText('查看所有稅額組合')
    const scenarioDialog = getLatestScenarioDialog()
    expect(scenarioDialog?.textContent).toContain('所有稅額組合')
    expect(scenarioDialog?.textContent).toContain('依您填寫的項目，共有')
    expect(scenarioDialog?.textContent).toContain('了解試算規則與稅率級距')
    const rulesLink = Array.from(scenarioDialog?.querySelectorAll<HTMLAnchorElement>('a') ?? [])
      .find((link) => link.textContent?.includes('試算規則'))
    const bracketLink = Array.from(scenarioDialog?.querySelectorAll<HTMLAnchorElement>('a') ?? [])
      .find((link) => link.textContent?.includes('稅率級距'))
    expect(rulesLink?.className).toContain('underline')
    expect(bracketLink?.className).toContain('underline')
    clickLatestScenarioDialogLinkByText('稅率級距')

    const dialog = getLatestFormulaDialog()
    expect(dialog?.textContent).toContain('稅率級距')
  ...
  it('opens scenario rules from summary scenario dialog without duplicating scenario formulas', () => {
    ...
    expect(dialog?.textContent).toContain('本人各類所得分開：本人全部所得單獨計稅')
    expect(dialog?.textContent).toContain('配偶各類所得分開：配偶全部所得單獨計稅')
    ...
    const sourceLink = dialog?.querySelector<HTMLAnchorElement>('a[href*="tax-saving-manual"]')
    expect(sourceLink?.textContent).toBe('財政部稅務入口網')
