...
| `activeItemIds` | `Set<string>` — current checklist cards on result page (supports independent add/remove) |
...
- Helpers: `getEffectiveState`, `getActiveItemIdsFromSelectedSituations`, `getAddableSituationGroups`, `getGroupedItemsByActiveItemIds`, `getScrollTargetItemIdAfterAdd`, `getItemSourceSituationLabelsById`

## Situation toggle

- `toggleSituation(id)`: updates `selected`; when on selecting page, synchronizes `activeItemIds` from `selected`.
...
- **`handleAddSituations(ids)`**: merge ids into `selected`, rebuild `activeItemIds` from merged `selected`, and apply only when at least one new card will be added (`hasAddedItems` guard). Sets `scrollToItemId` from active-item diff (first newly visible card in render order).
- Add modal uses strict addable filtering: a situation is listed only when it can add at least one missing card to current `activeItemIds`.
...
- **`applyRemovalEffect`**: remove target id from `activeItemIds` and clear that card's entry from `cardInputMap`.
- Remove action does not mutate `selected`; users can still manage situations independently on page one.
...
3. **`storage` listener** for `SITUATION_SELECTION_STORAGE_KEY`: `parseSavedSituationSelection` → `setSelected` + rebuild `activeItemIds`; if empty, `appState` → `selecting`.
